...
export default defineEventHandler(async (event) => {
    ...
    const index = data.games.findIndex((g: any) => g._id === id)
    if (index === -1) {
        throw createError({ statusCode: 404, statusMessage: 'Game not found' })
    }
    const existing = data.games[index]

    let romPublicPath = existing.path
    if (romFile) {
        ...
        await fs.writeFile(path.join(romDir, romFilename), romFile.data)
        romPublicPath = `/roms/${platform}/${romFilename}`
    }

    let cover_url = coverUrlField || existing.cover_url || ''
    if (coverFile) {
